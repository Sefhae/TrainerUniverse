...
import type { Review, Trainer } from '../types';
  ...
  return (
    <div>
      <p
        className={cn(
          'eyebrow',
          tone === 'dark' ? 'text-volt' : 'text-ink/50'
        )}
      >
    ...
    return (
      <div className="min-h-[78vh] bg-ink">
        <EmptyState
          ...
          action={
            <Link to="/trainers" className="btn btn-volt">
              Browse All Trainers
            </Link>
          }
        />
              ...
              {trainer.certifications.length > 0 ? (
                <ul className="mt-8 space-y-3">
                  {trainer.certifications.map((cert) => (
                    <li
                      key={cert.id}
                      className="flex items-start gap-4 border border-ink/10 bg-white p-4"
                    >
