...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (action === 'approve') {
    await db.transaction(async (tx) => {
      await tx.prepare('DELETE FROM trainer_students WHERE trainer_id = ? AND student_id = ?')
        .run(reqRow.trainer_id, reqRow.student_id);
      await tx.prepare(`UPDATE student_removal_requests SET status = 'approved' WHERE id = ?`).run(requestId);
    });
  } else if (action === 'reject') {
    await db.prepare(`UPDATE student_removal_requests SET status = 'rejected' WHERE id = ?`).run(requestId);
  } else {
