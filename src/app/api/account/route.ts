...
export async function PUT(req: NextRequest) {
  ...
  const user = await db.prepare('SELECT id, email, password_hash FROM users WHERE id = ?').get(p.userId) as
  ...
  if (newEmail && newEmail !== user.email) {
    ...
    const conflict = await db.prepare('SELECT id FROM users WHERE email = ? AND id != ?').get(newEmail, user.id);
    if (conflict) return NextResponse.json({ error: 'That email is already in use.' }, { status: 409 });
    await db.prepare('UPDATE users SET email = ? WHERE id = ?').run(newEmail, user.id);
  }
