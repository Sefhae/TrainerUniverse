...
export async function PUT(req: NextRequest, { params }: Params) {
  ...
  const updated = await db.prepare('SELECT * FROM previous_work WHERE id = ?').get(Number(workId)) as Record<string, unknown>;
...
export async function DELETE(req: NextRequest, { params }: Params) {
  ...
  await db.prepare('DELETE FROM previous_work WHERE id = ?').run(Number(workId));
