...
export async function GET(req: NextRequest) {
  ...
  if (payload.role === 'trainer' && payload.trainerId) {
    const rows = await sessionsForTrainer(payload.trainerId) as Record<string, unknown>[];
    return NextResponse.json(rows.map(mapSession));
  }

  if (payload.role === 'student' && payload.studentId) {
    const rows = await sessionsForStudent(payload.studentId) as Record<string, unknown>[];
    return NextResponse.json(rows.map(mapSession));
  }
