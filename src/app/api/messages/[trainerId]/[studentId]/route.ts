...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    const trainerExists = await db.prepare('SELECT id FROM trainer_profiles WHERE id = ?').get(tId);
    const studentExists = await db.prepare('SELECT id FROM student_profiles WHERE id = ?').get(sId);
