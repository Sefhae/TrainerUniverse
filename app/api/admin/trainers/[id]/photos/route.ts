...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  if (profilePhoto instanceof File) {
    ...
    db.prepare('UPDATE trainer_profiles SET profile_photo = ? WHERE id = ?').run(photoPath, Number(id));
  ...
  if (coverPhoto instanceof File) {
    ...
    db.prepare('UPDATE trainer_profiles SET cover_photo = ? WHERE id = ?').run(photoPath, Number(id));
