...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (action === 'approve') {
    db.transaction(() => {
      db.prepare(`UPDATE session_requests SET status = 'approved' WHERE id = ?`).run(requestId);
      // Enroll student if not already enrolled
      db.prepare(`
        INSERT OR IGNORE INTO trainer_students (trainer_id, student_id) VALUES (?, ?)
      `).run(payload.trainerId, row.student_id);
    })();
  } else {
    db.prepare(`UPDATE session_requests SET status = 'rejected' WHERE id = ?`).run(requestId);
  }
